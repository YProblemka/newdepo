(function ($) {
    ...
    $(function () {
        ...
        $("article.content")
            .find("table")
            .wrap('<div class="for-mobile-view"></div>');
            ...
            if (styles[i].href && styles[i].href.indexOf("styles.css") !== -1) {
                for (var j = 0; j < styles[i].cssRules.length; j++) {
                    if (
                        ...
                        styles[i].cssRules[j].style.width == "100vw"
                    ) {
            ...
            click: function (menuWrap, item, settings) {
                ...
                function menuToggle() {
                    if (subMenu.is(":visible")) {
                        ...
                        item.siblings()
                            .find("> a ." + settings.childIconsClass)
                            .removeClass("hover");
                        // Определение положения выпадашки чтобы не выводилось за пределы окна
                        pluginObj._menuPosition(subMenu);
    ...
    Plugin.prototype = {
        init: function () {
            ...
            if (window.matchMedia("all and (max-width: 480px)").matches) {
                ...
                window.matchMedia("all and (max-width: 640px)").matches
            ) {
            ...
            if (
                window.side_panel_controller &&
                side_panel_controller.inPanel(widgetClass)
            ) {
                moreButFlag = false;
            }
            ...
            if (!!menuClass.match(/horizontal/i)) {
                ...
                if (!moreBut.length) {
                    moreBut = $(
                        '<li class="more-button"><a href="#"><span>' +
                            settings.moreText +
                            "</span></a></li>"
                    ); // data-trigger="click"
                ...
                var firstLevelItems = menuMainUl
                    .children(":visible")
                    .not(".more-button");
                moreMenu.html("");
            ...
            if (!!settings.childIcons) {
                ...
                var childIconHTML =
                    '<em class="' + settings.childIconsClass + '"></em>';

                if (mShow === "treemenu") {
                    childIconHTML =
                        '<em class="' +
                        settings.childIconsClass +
                        '"><b style="position: absolute; top: 50%; left: 50%; width: 30px; height: 30px; margin: -15px 0 0 -15px;"></b></em>';
                }
    ...
    function init() {
        $search.each(function () {
            ...
            if ($this.attr("data-setting-animate_list")) {
                ...
                animate_list_arr = $this
                    .attr("data-setting-animate_list")
                    .split(",");
                ...
                if (defaults.animation == "slide") {
                    ...
                    if (iebrw == 9 || iebrw == 10 || iebrw == 11) {
                        ...
                        $(document).on("click." + clss, function (event) {
                            if (
                                $(event.target).closest(".wm-search-inner")
                                    .length
                            )
                                return;
                            inp.fadeOut();
...
$(function () {
    ...
    $(".slider").each(function () {
        ...
        options.autoControlsSelector = $this.attr(
            "data-setting-auto_controls_selector"
        );
        options.autoControls = !!parseInt(
            $this.attr("data-setting-auto_controls").split(",")[0]
        );
        options.autoControlsCombine = !!parseInt(
            $this.attr("data-setting-autoControlsCombine")
        );
        ...
        function init() {
            ...
            if ($this.find(".slider-item .overfill").length == 0) {
                $this
                    .find(".slider-item")
                    .prepend('<div class="overfill"></div>');
            }
            ...
            if (textPosition == "out" && force) {
                ...
                options.onSlideBefore = function (
                    $slideElement,
                    oldIndex,
                    newIndex
                ) {
                    if (
                        (newIndex == 0 &&
                            oldIndex == textSlider.getSlideCount() - 1) ||
                        newIndex - oldIndex == 1
                    ) {
                        textSlider.goToNextSlide();
                    } else if (
                        (oldIndex == 0 &&
                            newIndex == textSlider.getSlideCount() - 1) ||
                        oldIndex - newIndex == 1
                    ) {
                        textSlider.goToPrevSlide();
                    } else {
                ...
                textPosition == "in" &&
                $this.find(">.wrap_text_dm").length > 0
            ) {
            ...
            if (itemLength > 1) {
                if (bxslider) {
                    if (
                        options.pager &&
                        $pager_wrap.find(".slider-pager").length == 0
                    ) {
                        if ($pager_wrap.find(".bx-auto_controls").length == 0) {
                            $pager_wrap.append($new_pagers_html);
                        }
                        $pager_wrap
                            .find(".bx-auto_controls")
                            .append($pager_wrap_inner);
...
$(function () {
    ...
    $(".news-block").each(function () {
        ...
        if (slider_arr.indexOf("1") > -1 && $.fn.bxSlider) {
            options.pagerSelector = $newsblock.attr(
                "data-setting-pager_selector"
            );
            options.prevSelector = $newsblock.attr(
                "data-setting-prev_selector"
            );
            options.nextSelector = $newsblock.attr(
            ...
            options.controls = !!parseInt(
                $newsblock.attr("data-setting-controls")
            );
            options.pager = !!parseInt(
                $newsblock.attr("data-setting-pager").split(",")[0]
            );
            options.pause =
                parseInt($newsblock.attr("data-setting-pause"), 10) || 4000;
            ...
            options.maxSlides =
                parseInt(
                    $newsblock.attr("data-setting-count").split(",")[0],
                    10
                ) || 1;
            options.minSlides =
                options.mode === "horizontal" ? 1 : options.maxSlides;
            options.moveSlides = $newsblock.attr("data-setting-move") || 0;
            options.autoControlsSelector = $newsblock.attr(
                "data-setting-auto_controls_selector"
            );
            options.autoControls = !!parseInt(
                $newsblock.attr("data-setting-auto_controls").split(",")[0]
            );
            options.autoControlsCombine = !!parseInt(
                $newsblock.attr("data-setting-autoControlsCombine")
            );
            options.autoHover = true;
            ...
            auto_controls_arr = $newsblock
                .attr("data-setting-auto_controls")
                .split(",");
            ...
            function init(force) {
                ...
                var newContainerWidth = $newsblock.width(),
                    itemHeight,
                    viewportHeight;
                ...
                count = parseInt(
                    $newsblock.attr("data-setting-count").split(",")[media_ind],
                    10
                );
                isJustify =
                ...
                if (auto_controls_arr.length > 1) {
                    options.autoControls = !!parseInt(
                        auto_controls_arr[media_ind]
                    );
                }

                $(options.pagerSelector).css({
                    display: options.pager == true ? "flex" : "none",
                });
                $newsblock.find(".controls").css({
                    display: options.controls == true ? "flex" : "none",
                });

                if (options.mode === "vertical") {
                    itemHeight = Math.max.apply(
                        Math,
                        $items
                            .map(function () {
                                return $(this).height();
                    ...
                    setting_columns =
                        col_arr[media_ind] == "auto"
                            ? false
                            : parseInt(col_arr[media_ind]);
                    if (setting_columns) {
                        count = setting_columns;
                    }
                    options.maxSlides = Math.min(
                        count,
                        Math.ceil($items.parent().width() / $items.width())
                    );
                }
                ...
                if (
                    $items.length > options.maxSlides &&
                    slider_arr[media_ind] == 1
                ) {
                    ...
                    if (bxslider) {
                        ...
                        $list.data(
                            "bxslider",
                            (bxslider = $list.bxSlider(options))
                        );
                    }
                    if (options.pager) {
                ...
                if (self.find("[type=hidden]").val()) {
                    data =
                        typeof JSON.parse == "function"
                            ? JSON.parse(self.find("[type=hidden]").val())
                            : eval(self.find("[type=hidden]").val());
                }
                var params = {
                ...
                if (data) {
                    for (var i in data) {
                        var myPlacemark = new ymaps.Placemark(
                            data[i].point.split(",").map(function (e) {
                                return $.trim(e);
                            }),
                            {
                                iconContent: $("<div>")
                                    .html(data[i].iconContent)
                                    .text(),
                                balloonContent: $("<div>")
                                    .html(data[i].balloonContent)
                                    .html(),
                            },
                            {
...
(function (exports) {
    ...
    VelocityTracker.prototype.pruneHistory_ = function (timeStamp) {
        for (var i = 0; i < this.recentTouchMoves_.length; ++i) {
            if (
                this.recentTouchMoves_[i].timeStamp >
                timeStamp - VelocityTracker.kTimeWindow
            ) {
    ...
    VelocityTracker.prototype.update_ = function (e) {
        ...
        var deltaX =
            ...
            oldestTouchMove.changedTouches[0].clientX;
        var deltaY =
            e.changedTouches[0].clientY -
            oldestTouchMove.changedTouches[0].clientY;
    ...
    function DrawerController(options) {
        ...
        this.defaultAnimationSpeed =
            (this.right - this.left) / DrawerController.kBaseSettleDurationMS;
    ...
    DrawerController.kTouchSlopSquare =
        DrawerController.kTouchSlop * DrawerController.kTouchSlop;
    ...
    DrawerController.prototype.targetPosition = function (deltaT) {
        if (this.state == DrawerController.kFlinging)
            return (
                this.positionAnimationBase + this.animationVelocityX * deltaT
            );
        var targetFraction = this.curve.scaleTime(
            deltaT / this.animationDuration
        );
        var animationWidth =
            this.state == DrawerController.kOpening
                ? this.width - this.positionAnimationBase
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    Object.keys(layers).forEach(function (clss) {
        ...
        if (elements[clss]) {
            elements[clss].parentNode.insertBefore(
                comments[clss],
                elements[clss]
            );
        } else {
    ...
    side_panel_content_inner =
        side_panel.querySelector(".side-panel-content-inner") ||
        side_panel_content;
    ...
    window.side_panel_controller = side_panel_controller = new DrawerController(
        {
            ...
            willOpen: function () {
                ...
                if (
                    ...
                    document.body.style.borderRight =
                        bodyScrollBarWidth + "px solid transparent";
                }
            },
            didClose: function () {
                side_panel_mask.style.display = "none";
            ...
            onAnimate: function (position) {
                ...
                side_panel_content.style.transform =
                    "translate3d(" +
                    (flip ? rightEdge - position : position - rightEdge) +
                    "px,0,0)";
            },
        }
    );
